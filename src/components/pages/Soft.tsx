...
import {
    ...
    useIsRequestedByID, useIsInBasketByID, useIsAuthenticated, setCurrentSoft, useIsStaff
                        ...
                        <h2 className="mb-0 d-flex flex-column flex-sm-row justify-between sm text-center">{soft.name}
                            <div className="mt-10 mt-sm-0">
                                {!isAuthenticated ?
                                    ...
                                    <div className="mt-10 mt-sm-0">
                                        {isStaff ? <Button className="" variant="success"
                                                           href={`#/update_soft/${softId}`}>Редактировать</Button> : ''}
                                        {isStaff ? '' : (
                                            isRequestedBefore ?
                                                <Button className="w-100" href={`#/my_payments/`}>Просмотреть в
                                                    заказах</Button> :
                        ...
                        <div className="w-100 text-center">
                            <div className="mt-15 d-inline-block">
                                {!(soft.files && soft.files.length) ? <h2>...</h2> :
                                    soft.files.map(file => (
                                        //@ts-ignore
                                        <FileCard file={file} saveEnabled={isAviable || isStaff} deleteEnabled={false} />)
                                    )
                                }
                            </div>
                        </div>
